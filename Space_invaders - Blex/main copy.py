...
def saugage_bullet(x, y):
    global saugage_state
    sausage_state = "buff_mode"
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                playerX_movement += -2.5
            if event.key == pygame.K_RIGHT:
                playerX_movement += 2.5
            # Bullet from player
            if event.key == pygame.K_SPACE:
                if bullet_state is "ready" and saugage_state is "ready":
                    if k == 0:
            ...
            if event.key == pygame.K_LCTRL:
                if k == 1:
                    if saugage_state is "ready":
                        bullet_sound = mixer.Sound('laser.wav')
                        bullet_sound.play()
                        bulletX = playerX
                        saugage_bullet(bulletX, bulletY)
                        #saugage_bullet(bulletX - 15, bulletY)
                        #saugage_bullet(bulletX + 15, bulletY)
                elif k == 0:
                    pass
    ...
    for i in range(num_of_tomatoes):
        ...
        if s_collision and score_value >= 3 and score_value <= 7:
            saugage_state = "ready"
            k = 1
            not_picked = True
            print("oof")
